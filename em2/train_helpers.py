...
def train(train_loader, D, G, disc_optimizer, gen_optimizer, z_size, n_epochs, print_every=50, index_models_to_save=[], models_folder=''):
    '''Trains adversarial networks for some number of epochs,
        ...
        param, index_models_to_save: each index is a reference to the epoc of the target model to save
        param, models_folder: folder where all model snapshoots are going to be saved
    ...
    for epoch in range(1, n_epochs+1):
        ...
        # seGANs check: should model be saved depending of epoch?
        if epoch in index_models_to_save:
            model_path = f'{models_folder}/segan{epoch}'
            print(f' *** saving model {epoch}: {model_path}')
